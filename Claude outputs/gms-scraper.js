...
async function upsertFixtureDetail(env, fixtureId, gmsFixtureUuid, detail, now) {
  if (!detail) return;
  await env.DB.prepare(`
    INSERT INTO fixture_details (
      fixture_id, gms_fixture_uuid, competition_name, venue_address, status,
    ...
    ON CONFLICT(fixture_id) DO UPDATE SET
      ...
      last_updated=excluded.last_updated
  `).bind(
    fixtureId, gmsFixtureUuid, detail.competition_name, detail.venue_address, detail.status,
...
async function runFixtureDetailsStage(env) {
  ...
  const { results: candidates } = await env.DB.prepare(`
    SELECT f.fixture_id, f.gms_fixture_uuid, d.status, d.last_updated
    FROM fixtures f
  ...
  for (const row of candidates || []) {
    ...
    try {
      ...
      await upsertFixtureDetail(env, row.fixture_id, row.gms_fixture_uuid, detail, now);
      fetched++;
    } catch (err) {
      errored++;
